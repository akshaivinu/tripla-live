        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {paths.map((path, i) => (
            <motion.div
              ...
              className={`p-10 rounded-[2.5rem] glass border border-white/5 flex flex-col justify-between group hover:border-white/20 transition-all h-[450px]`}
            >
              <div>
                <h3 className="text-3xl font-bold outfit uppercase text-white mb-6 leading-tight">{path.title}</h3>
                <p className="text-zinc-400 text-lg font-light leading-relaxed">{path.desc}</p>
              </div>
              
              <button className="flex items-center justify-between w-full p-6 bg-white rounded-2xl text-black font-bold uppercase tracking-widest text-sm hover:bg-zinc-200 transition-all">
                {path.cta}
                <ArrowRight size={20} />
              </button>
            </motion.div>
